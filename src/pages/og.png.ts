...
export const GET: APIRoute = async function get({ request }) {
  ...
  const title = url.searchParams.get("title") || "Chenxi Huang";
  const description =
    url.searchParams.get("description") ||
    "A personal knowledge garden about AI, agents, product design, and knowledge management.";

  // Keep OG generation deterministic in CI by reading the bundled OFL font.
  const LatoRegular = await fs.readFile("./public/fonts/Lato-Regular.ttf");

  // Load and process Chenxi's profile image
  const {
    base64: coverImage,
    width,
    height,
  } = await getImageData("./public/images/profile/chenxi-huang-github.jpg");
